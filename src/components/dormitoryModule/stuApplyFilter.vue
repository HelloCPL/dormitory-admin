    ...
    <my-filter-box>
      <div class="we-padding">
        <el-form ref="form" :model="dataInfo" label-position="left" label-width="100px" class="we-flex-wrap">
          ...
          <el-form-item label="存放状态" class="we-width-300 we-margin-right-20">
            <el-select v-model="dataInfo.replyStatus" clearable placeholder="请选择">
              <el-option v-for="item in filterData.replyStatusList" :key="item.id" :label="item.name" :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
...
export default {
  data() {
    return {
      dataInfo: {
        ...
        dorRoomId: null,
        type: [], // 类型 多选
        replyStatus: null, // 展示状态

      },
      filterData: {
        optionsBuilding: [], // 宿舍栋选择列表
        optionsRoom: [], // 宿舍房间选择列表
        replyStatusList: [{ id: 1, name: '待审核' }, { id: 2, name: '待处理' }, { id: 3, name: '已处理' }, { id: 0, name: '审核不通过' }],
        typeList: [{ id: 1, name: '来访人员' }, { id: 2, name: '出入品' }, { id: 3, name: '托管' }, { id: 4, name: '其他' }]
      },
      loading: false,
    }
  },
  ...
  methods: {
    ...
    onCancel() {
      ...
      this.dataInfo.replyStatus = null
      this.dataInfo.type = []
    },
